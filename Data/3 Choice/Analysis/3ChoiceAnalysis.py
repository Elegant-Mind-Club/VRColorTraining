...
file_path = '/Users/Mingda/Library/Mobile Documents/com~apple~CloudDocs/Documents/UCLA/Research/Github/VRColorTraining/Data/3 Choice/Sample_Reaction_Time_Data.csv'
...
# Plot for blueCircle trials
blue_trials = filtered_data[filtered_data['StimType'] == 'blueCircle']
mean_blue = blue_trials['AdjustedReactionTime_ObjectShowTime'].mean()
std_blue = blue_trials['AdjustedReactionTime_ObjectShowTime'].std()

# Histogram data for blueCircle
hist_blue, bin_edges_blue = np.histogram(blue_trials['AdjustedReactionTime_ObjectShowTime'], bins=range(int(blue_trials['AdjustedReactionTime_ObjectShowTime'].min()), 
                  int(blue_trials['AdjustedReactionTime_ObjectShowTime'].max()) + 25, 25), density=True)

bin_centers_blue = (bin_edges_blue[:-1] + bin_edges_blue[1:]) / 2

# Fit Gaussian for blueCircle
popt_blue, _ = curve_fit(gaussian, bin_centers_blue, hist_blue, p0=[1, mean_blue, std_blue])

# Extract the mean and standard deviation of the fitted Gaussian for blueCircle
fitted_amp_blue, fitted_mean_blue, fitted_std_blue = popt_blue

# Plot the histogram and the fitted Gaussian for blueCircle
sns.histplot(blue_trials['AdjustedReactionTime_ObjectShowTime'], bins=bin_edges_blue, kde=False, color='#8080FF', stat='density')
plt.axvline(mean_blue, color='#000080', linestyle='solid', linewidth=1, label=f'Mean: {mean_blue:.2f} ms')
plt.axvline(mean_blue + std_blue, color='#0000FF', linestyle='dashed', linewidth=1, label=f'Standard Deviation: {std_blue:.2f} ms')
plt.axvline(mean_blue - std_blue, color='#0000FF', linestyle='dashed', linewidth=1)

# Plot Gaussian fit for blueCircle
x_fit_blue = np.linspace(mean_blue - 4*std_blue, mean_blue + 4*std_blue, 1000)
y_fit_blue = gaussian(x_fit_blue, *popt_blue)
plt.plot(x_fit_blue, y_fit_blue, color='#0000FF', linewidth=2, label=f'Gaussian Fit: μ = {fitted_mean_blue:.2f} ms, σ = {fitted_std_blue:.2f} ms')





plt.title('Red, Green, and Blue Reaction Time')
